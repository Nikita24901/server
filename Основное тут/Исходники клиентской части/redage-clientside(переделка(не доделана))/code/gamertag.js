...
function preDraw()
{
    ...
    mp.game.player.getEntityPlayerIsFreeAimingAt(playerAimAt);
    mp.game.player.getPlayerTargetEntity(playerTarget);
}
            ...
            if(distance < 10.0)
            {
                if(player.getVariable('INVISIBLE') != true && player.getVariable('HideNick') != true)
                {
					...
					if(player.getVariable('IS_MASK') == true) {
						text = (localplayer.getVariable('IS_ADMIN') === true) ? `${player.name} (${player.remoteId}${passportText})` : `ID: ${player.remoteId}`;
					} else {
						text = (localplayer.getVariable('IS_ADMIN') === true || friends[player.name] != undefined || passports[player.name] != undefined) ? `${player.name} (${player.remoteId}${passportText})` : `ID: ${player.remoteId}`;
					}
        
                    if (localplayer.getVariable('fraction') != null && player.getVariable('fraction') != null && localplayer.getVariable('fraction') === player.getVariable('fraction'))  text = `${player.name} (${player.remoteId}${passportText})`;
...
function drawPlayerTag(player, x, y, distance, displayname, color)
{
    ...
    if (player.handle == playerTarget || player.handle == playerAimAt || spectating)
    {
        y += 0.04;
